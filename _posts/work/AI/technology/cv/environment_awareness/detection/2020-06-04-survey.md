...
- 具体的，feature map 上一个点对应一组 anchor（9个），然后用 1×1 进行分类和回归`1×1用的好`{: .notice--warning}；    
...
```
把 RoIPooling 后的 fc 前移动（或者说去掉了），检测效果差，因为卷积具有平移不变性，不利于定位`瞎说的吧，平移不变是对不同的图说的吧，要不然为啥到处用 FPN 融合位置信息呢`{: .notice--warning}，于是设计了位置敏感得分地图；    
...
backbone： FasterRCNN
```
**动机**：由于 IoU 阈值的大小对检测精度有着重要影响；尤其是候选框在 IoU 阈值附近时，检测效果比其他 IoU 好很多；`为啥呢`{: .notice--warning}；    
...
特征融合前用了 BN，达到归一化感受野和不同尺度的效果`怎么就达到这个效果了，确定不是为了强行使用 BN 而杜撰的概念`{: .notice--warning}
...
**缺点**：    
1. 定位误差大`归咎于下采样率高`{: .notice--warning}，小目标效果差、召回率低（*损失函数没着重考虑小目标，下采样率高，没有 FPN*）；   
